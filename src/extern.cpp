...
#include <nil/crypto3/codec/extern/detail/wraps.hpp>
#include <nil/crypto3/codec/extern/detail/memops.hpp>
...
int nil_crypto3_hex_encode(const uint8_t *in, size_t len, char *out, uint32_t flags) {
...
int nil_crypto3_base32_encode(const uint8_t *in, size_t len, char *out, size_t *out_len) {
    return ffi_guard_thunk(BOOST_CURRENT_FUNCTION, [=]() -> int {
        const std::string base64 = encode<codec::base<32>>(in, in + len);
...
int nil_crypto3_base32_decode(const char *base64_str, size_t in_len, uint8_t *out, size_t *out_len) {
    return ffi_guard_thunk(BOOST_CURRENT_FUNCTION, [=]() -> int {
        ...
        decode<codec::base<32>>(base64_str, base64_str + in_len, out);
        return CRYPTO3_FFI_SUCCESS;
    });
}

int nil_crypto3_base64_encode(const uint8_t *in, size_t len, char *out, size_t *out_len) {
    return ffi_guard_thunk(BOOST_CURRENT_FUNCTION, [=]() -> int {
...
int nil_crypto3_base58_encode(const uint8_t *in, size_t len, char *out, size_t *out_len) {
    return ffi_guard_thunk(BOOST_CURRENT_FUNCTION, [=]() -> int {
        const std::string base64 = encode<codec::base<58>>(in, in + len);
        return ffi::write_str_output(out, out_len, base64);
    });
}
...
int nil_crypto3_base58_decode(const char *base64_str, size_t in_len, uint8_t *out, size_t *out_len) {
    return ffi_guard_thunk(BOOST_CURRENT_FUNCTION, [=]() -> int {
        ...
        decode<codec::base<58>>(base64_str, base64_str + in_len, out);
        return CRYPTO3_FFI_SUCCESS;
    });
}
}